...
export default function Celebrate() {
  ...
  const isEmpty = completedGoals.length === 0;

  // Stagger choreography driven from a single cappedStagger source so the
  // lock-in beat timing stays in lockstep with the goal-card entrances.
  const staggerCfg = cappedStagger(completedGoals.length, 0.5);
  const perChild = staggerCfg.staggerChildren;
  const stairDelayChildren = 0.4;
  const lockInDelay = reduced
    ? 0
    : stairDelayChildren + perChild * Math.max(completedGoals.length, 1) + 0.25;

  // Prefetch the confetti chunk on mount so the lock-in beat (fired several
  // seconds later) doesn't pay the import cost at the critical moment.
  useEffect(() => {
    prefetchConfetti();
  }, []);

  // Lock-in beat: fires confetti + pulses the hero card at the end of the
  // stagger sequence. Suppressed when there are no completed goals or when
      ...
      if (heroCardRef.current) {
        animateElement(
          heroCardRef.current,
          {
            scale: [1, 1.04, 1],
            boxShadow: [
              '0 0 32px var(--secondary-accent-glow)',
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 relative">
      ...
      <div className="text-center py-8 md:py-12 relative z-10">
        ...
        >
          {isEmpty ? (
            ...
            <>
              You've completed {completedGoals.length} goal{completedGoals.length > 1 ? 's' : ''} and saved{' '}
              <CountUp value={totalSaved} format={fmt} delay={0.5} />!
            </>
          )}
        </motion.p>
      </div>
          ...
          variants={{
            ...
            visible: {
              transition: reduced
                ...
                : { staggerChildren: perChild, delayChildren: stairDelayChildren },
            },
          }}
        >
          {completedGoals.map((goal, i) => {
            const monthly = Math.round(goal.targetAmount / Math.max(1, goal.timelineMonths));
            const baseDelay = stairDelayChildren + perChild * i;
            return (
              <motion.div
                key={goal.id}
                ref={i === 0 ? heroCardRef : undefined}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
                    <div className="p-3 rounded-xl" style={{ background: 'var(--surface-hover)' }}>
                      ...
                      <div className="text-lg font-bold slashed-zero text-[var(--card-foreground)]" style={{ fontFamily: 'var(--font-display)' }}>
                        <CountUp value={goal.targetAmount} format={fmt} delay={baseDelay + 0.1} />
                      </div>
                    </div>
                    <div className="p-3 rounded-xl" style={{ background: 'var(--surface-hover)' }}>
                      <div className="text-xs text-[var(--secondary)] mb-1" style={{ fontFamily: 'var(--font-body)' }}>Timeline</div>
                      <div className="text-lg font-bold slashed-zero text-[var(--card-foreground)]" style={{ fontFamily: 'var(--font-display)' }}>
                        <CountUp value={goal.timelineMonths} format={(n) => `${Math.round(n)} months`} delay={baseDelay + 0.15} />
                      </div>
                    </div>
                    <div className="p-3 rounded-xl hidden md:block" style={{ background: 'var(--surface-hover)' }}>
                      ...
                      <div className="text-lg font-bold slashed-zero text-[var(--card-foreground)]" style={{ fontFamily: 'var(--font-display)' }}>
                        <CountUp value={monthly} format={fmt} delay={baseDelay + 0.2} />
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* Net Worth Before/After — hidden when no completed goals (the empty state below replaces it) */}
      {!isEmpty && (
        <motion.div
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-2xl md:text-3xl font-bold slashed-zero" style={{ fontFamily: 'var(--font-display)', color: 'var(--secondary-accent-text)' }}>
                <CountUp value={savings + investments + totalSaved} format={fmt} delay={lockInDelay} />
              </div>
            </div>
          </div>
      ...
      {/* Empty state — designed alternative when no goals have been completed */}
      {isEmpty && (
        <motion.div
          className="bento-card p-8 text-center max-w-md mx-auto relative z-10"
          variants={cardEntry}
          initial="initial"
          animate="animate"
        >
          <Trophy size={64} className="mx-auto mb-4" style={{ color: 'var(--secondary)', opacity: 0.4 }} />
          <h2 className="font-bold mb-2 text-[var(--card-foreground)]" style={{ fontSize: 'var(--text-2xl)', fontFamily: 'var(--font-display)' }}>
            No milestones yet
          </h2>
          <p className="text-[var(--secondary)] mb-6" style={{ fontSize: 'var(--text-base)', fontFamily: 'var(--font-body)' }}>
            Complete a goal in your Journey to unlock celebrations.
          </p>
          <button
            onClick={() => navigate('/journey')}
            className="py-3 px-6 rounded-xl font-bold inline-flex items-center justify-center gap-2 button-press"
            style={{
