...
const CATEGORY_STYLE: Record<
  string,
  { icon: string; color: string; subtle: string; text: string }
> = {
  bike: {
    icon: 'Bike',
    color: 'var(--accent)',
...
export default function Dashboard({ onPennyClick }: { onPennyClick: () => void }) {
  ...
  const badges = [
    ...
    },
    {
      ...
      earned: healthScore && healthScore.overall >= 50,
    },
    {
      name: 'Goal Setter',
    ...
    },
    {
      ...
      color: 'var(--red)',
      desc: 'All goals checked this month',
      earned: goals.length > 0 && goals.every((g) => g.checkedThisMonth || g.status === 'complete'),
    },
    {
      name: 'Wealth Builder',
                ...
                <div className="goals-header">
                  <div>
                    ...
                    <p className="goals-subtitle">
                      ...
                      {goals.filter((g) => g.status === 'complete').length} completed
                    </p>
                  </div>
                  <button
                    className="pill"
                ...
                <div className="goals-list">
                  {activeGoals.map((g) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => navigate('/journey')}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') navigate('/journey');
                        }}
                      >
                        <div
                          className="goal-icon"
                          style={{ background: cat.subtle, color: cat.color }}
                        >
                        ...
                        <div className="goal-name-col">
                          ...
                          <p className="goal-sub">
                            {formatInr(monthly)}/mo · {g.timelineMonths} mo left
                          </p>
                        </div>
                        ...
                        <div className="goal-bar">
                          ...
                          <div
                            ...
                            style={{
                              ...
                              boxShadow: `0 0 var(--space-1) ${cat.color}`,
                            }}
                          />
                        </div>
                        ...
                        <p className="slashed-zero goal-amounts">
                          <span className="goal-amounts-current">
                            {formatInrCompact(g.currentAmount)}
                          </span>
                          <span className="goal-amounts-target">
                            {' '}
                  ...
                  {activeGoals.length === 0 && (
                    <div className="goals-empty">
                      ...
                      <button className="pill active" onClick={() => navigate('/journey')}>
                        + Add a goal
                      </button>
                    </div>
                  )}
            ...
            <motion.div className="bento-card col-span-4 flex flex-col" variants={cardVariants}>
              ...
              {nextGoal ? (
                (() => {
                  ...
                  const monthly =
                    nextGoal.monthlyAllocation ||
                    Math.round(
                      (nextGoal.targetAmount - nextGoal.currentAmount) /
                        Math.max(1, nextGoal.timelineMonths),
                    );
                  return (
                    <>
                      ...
                      <p className="next-step-remaining">
                        {formatInrCompact(nextGoal.targetAmount - nextGoal.currentAmount)} left to
                        save
                      </p>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const newAmt = Math.min(
                            ...
                            nextGoal.currentAmount + m,
                          );
                          const justCompleted = newAmt >= nextGoal.targetAmount;
                          updateGoal(nextGoal.id, {
                            currentAmount: newAmt,
                            checkedThisMonth: true,
                            status: justCompleted ? 'complete' : 'in-progress',
                          });
                          const styles = getComputedStyle(document.documentElement);
                          const accent = styles.getPropertyValue('--accent').trim() || '#495bff';
                          const secondary =
                            styles.getPropertyValue('--secondary-accent').trim() || '#ac49ff';
                          const lime =
                            styles.getPropertyValue('--tertiary-accent').trim() || '#b0ff09';
            ...
            <motion.div className="bento-card col-span-7 flex gap-6" variants={cardVariants}>
              ...
              <div className="metrics-col">
                {(
                  [
                    [
                      period === 'This month' ? 'Monthly Income' : `${period} Income`,
                      animIncome,
                      '',
                    ],
                    [
                      period === 'This month' ? 'Monthly Surplus' : `${period} Surplus`,
                      animSurplus,
                      surplus > 0 ? '+' : '',
                    ],
            ...
            <motion.div className="bento-card col-span-5" variants={cardVariants}>
              ...
              <div className="activity-list">
                {goals.slice(0, 5).map((g) => {
                  ...
                  const monthly =
                    g.monthlyAllocation ||
                    Math.round((g.targetAmount - g.currentAmount) / Math.max(1, g.timelineMonths));
                  return (
                      ...
                      <div className="activity-left">
                        ...
                        <div>
                          ...
                          <p className="activity-status">
                            {g.checkedThisMonth ? 'Allocated' : 'Pending'}
                          </p>
                        </div>
                      </div>
                      <span className="slashed-zero activity-amount">
                        {formatInr(monthly)}
                        <span className="activity-period">/mo</span>
                      </span>
                    </div>
                  );
                })}
            ...
            <motion.div className="bento-card col-span-12" variants={cardVariants}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {badges.map((badge) => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-2 bg-[var(--surface-tint)] border border-[var(--border)]">
                      <badge.icon
                        size={20}
                    ...
                    <div className="text-xs font-bold text-[var(--card-foreground)] mb-1 font-body-family">
                      {badge.name}
                    </div>
                    <div className="text-[var(--text-2xs)] text-[var(--secondary)]">
                      {badge.desc}
                    </div>
                  </div>
                ))}
