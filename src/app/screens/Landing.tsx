...
export default function Landing({ isDark, setIsDark }: LandingProps) {
  ...
  return (
    <div className="h-full w-full flex flex-col p-4 md:p-8 relative overflow-x-hidden overflow-y-auto bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="max-w-6xl w-full m-auto relative z-10 flex flex-col items-center gap-6 md:gap-8 py-12">
        {/* Hero Section */}
        <div className="text-center space-y-4 max-w-3xl">
          <div className="flex items-center justify-center gap-3">
            <div className="w-3 h-3 rounded-full bg-[var(--lime)] shadow-[0_0_12px_var(--lime)]" />
            <span className="text-title slashed-zero">
              finpath
            </span>
          </div>

          <h1 className="text-hero slashed-zero">
            Every rupee has a <br />
            <span className="text-[var(--lime)]">destination</span><span className="text-[var(--lime)]">.</span>
          </h1>

          <p className="text-lg md:text-xl text-[var(--secondary)]">
            AI-powered finance companion for Indian professionals. Plan, track, and achieve your financial goals with clarity.
          </p>
        </div>

        {/* Bento Grid - Goals */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 w-full max-w-4xl mt-4">
          {goals.map((goal, i) => {
            const Icon = goal.icon;
            return (
              <div
                key={i}
                className="bento-card flex flex-col items-center text-center cursor-pointer transition-transform hover:-translate-y-1"
              >
                <div
                ...
                <h3 className="font-semibold text-sm md:text-base text-[var(--card-foreground)]">
                  {goal.title}
                </h3>
              </div>
            );
        ...
        <div className="mt-4">
          <button
            ...
            className="px-8 md:px-10 py-4 md:py-5 rounded-full font-bold text-base md:text-lg flex items-center gap-3 transition-transform hover:scale-105 active:scale-95 bg-[var(--lime)] text-[#050F1C] shadow-[0_8px_32px_rgba(176,255,9,0.3)]"
          >
            Start My Journey
